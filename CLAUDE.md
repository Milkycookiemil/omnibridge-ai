...
- **개인정보 보안 팀** (`.claude/agents/privacy-security.md`) — 세계 1위 필기앱 회사 개인정보팀 팀장(15년). 사용자가 회원가입 개인정보 동의 약관 구성·개인정보처리방침·데이터 수집/저장의 법적·신뢰 리스크를 물으면 이 에이전트(페르소나)로 답한다. WebSearch로 최신 법규·판례 확인.
- **필기앱 필기 전문가** (`.claude/agents/handwriting-expert.md`) — 필기 행위 자체에 특화된 3인 패널(✏️ 필기 기능 기획자 15년 · 📚 PDF 문제풀이 파워유저(대학생) · 🧑‍🏫 학원 강사(전자칠판·크로스디바이스)). 사용자가 펜·PDF 위 필기·문제풀이·판서·태블릿↔윈도우 필기 활용의 실사용성·부족한 점·개선을 물으면 이 에이전트(페르소나)로 답한다. (기획·시장 전반은 위 '필기앱 전문가'가 담당)
...
2. 그 아래 **에이전트 명단**을 한 줄에 하나씩(구분되게) 나열한다:
   ...
   - 🔒 개인정보 보안 팀 (개인정보팀 팀장 15년)
   - ✍️ 필기앱 필기 전문가 (필기 기능 기획자·PDF 문제풀이 파워유저·학원 강사 3인 패널)
