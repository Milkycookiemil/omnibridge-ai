# OmniBridge AI — 작업 진행 기록 (아카이브)

> 이 파일은 과거 세션 상세 로그·아키텍처·파일 변경 내역을 보관한다.
> **현재 재개 지점·다음 할 일은 [`PROGRESS.md`](./PROGRESS.md)** 상단을 볼 것.
> 필요한 내용만 `Grep`으로 키워드 검색해 읽는 것을 권장(통독은 토큰 낭비).

---

## 📜 2026-07-21 세션 상세 (팜리젝션 · 색상 상세 선택기 · 페이지 크기)

> ⚠️ **아래 팜리젝션의 `touch-action` 설명은 2026-07-22 핀치 줌 작업으로 대체됨.**
> 현재는 캔버스가 항상 `touch-action: none`이고, 손가락 팬/핀치를 앱이 직접 처리한다
> (펜모드 1손가락=수동 팬, 2손가락=핀치 줌+팬). "펜 감지 후 손가락은 그리지 않는다"는
> 팜리젝션 핵심 보장만 그대로 유지된다.

### 팜리젝션(스타일러스 우선) (2026-07-21) — 완료·검증 [전문가 리뷰 P0]
4개 전문 에이전트 전면 검토 결과 만장일치 최우선 문제였던 **손바닥/손가락이 획으로 남고 손가락으로 스크롤 못 하는 것**을 해결. `InkCanvas`·`PdfAdvancedRenderer` pointer 핸들러에 팜리젝션: **펜(스타일러스)이 한 번 감지되면(penMode) 그 뒤 손가락(touch)은 그리지 않고**, 캔버스 `touch-action`이 `none`→`pan-x pan-y`로 바뀌어 **손가락=네이티브 스크롤**이 된다. 펜을 안 쓰는 사용자는 처음부터 손가락으로 그릴 수 있다(폴백). 마우스는 항상 그림. 검증: 펜 전 손가락 그리기(3074px)→펜 그리기+penMode 발동→펜 후 손가락 0px(거부)+touch-action pan/마우스 다른 위치 1314px. tsc·build 통과.

### 색상 상세 선택기 (2026-07-21) — 완료·검증
퀵 팔레트(올가미 왼쪽 3색)에서 **이미 활성인 스와치를 한 번 더 탭하면** 삼성노트식 색상 상세 선택기가 열리고, **다시 탭하면 닫힘**(비활성 스와치 탭은 기존대로 적용). `ColorDetailPicker.tsx`: 표준 탭(색상×명도 그리드+무채색) / 사용자 지정 탭(채도·명도 사각 + hue 슬라이더, HSV 드래그) + 전/후 미리보기 + 색상코드(hex)·빨강/녹색/파랑 값 + 최근색(preferences.recentColors 최대6) + 스포이드(EyeDropper). 상세에서 색 고르면 그 슬롯 저장+펜 적용+최근색 추가. 검증: 활성 스와치 탭→상세 열림(#334155·RGB), 표준 그리드 빨강 선택→획 (230,55,55)·fav2 갱신·recent=['#e63737'], 사용자지정 HSV 렌더, 재탭 닫힘. tsc·build 통과.

### 페이지 크기 선택 (2026-07-21) — 완료·검증
빈 노트 생성 시 용지(무선/유선/옥스포드)에 더해 **페이지 크기** 선택 추가(레퍼런스 필기앱 참고). `pageSizes.ts`: 비율 A4·1:1·4:3·16:9·21:9 × 방향 세로/가로 → 캔버스 논리 크기(긴 변 1100 고정). 기본 **A4 세로**. NewNoteModal 서브스텝에 방향 토글+비율 칩+미리보기. **무마이그레이션 동기화**: Note.pageSize를 Supabase `style` 텍스트 컬럼에 `style|pageSize`로 인코딩(toRow encodeStyle/fromRow decodeStyle). LiveNoteView가 pageSize→`pageDims`를 InkCanvas width/height로 전달(획 좌표 공간). **구노트(pageSize 없음)는 800×800(1:1) 유지**. 검증: 16:9가로→캔버스 1100×619·획·IDB pageSize='16x9-l'·재열기 크기+획 복원, A4세로 기본 778×1100.
