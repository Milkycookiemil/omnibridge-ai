...
**로컬 커밋 1개 미푸시(페이지 크기 선택) · `tsc --noEmit` 0에러 · `npm run build` 통과.** 그 앞 HEAD=`c25b8fc`(연속 스크롤)까지는 배포됨.

> ✅ Supabase `transcript jsonb` 마이그레이션 완료(2026-07-20).
> **페이지 관리 완료**: 여러 페이지·삭제·순서변경·갤러리·연속스크롤↔플립·**페이지 크기 선택(A4/1:1/4:3/16:9/21:9 × 세로/가로)**.
> 실태블릿 확인 필요: 퀵 팔레트 길게 누르기·핀치 줌·연속스크롤 관성.
