...
- **② 툴바·팝오버 개편 — 완료·검증**: `PenTip.tsx`(펜종류별 실사풍 SVG 펜촉) + `PenToolbar` 팝오버 재작성. 팝오버 헤더에 실사 펜촉으로 종류 선택, `BubbleSlider`(숫자버블 슬라이더)로 굵기/투명도/필압, **투명도 슬라이더 신규 노출**(체커보드→색상 그라디언트 트랙 — 형광펜 등, 예전엔 opacity가 팝오버에 없어 고정이었음), 스포이드(EyeDropper API), 색상 행+256색. 실브라우저 검증: 펜촉5·슬라이더3(버블 "3"/"100%"/"1.0")·투명도 0.4로 획 반투명(maxAlpha 163)·형광펜 전환 시 노랑 팔레트+그라디언트 확인. tsc·build 통과.
  - **3색 퀵 팔레트 — 완료·검증(SIM 경고 3건 해소)**: `QuickColorPalette.tsx`(즐겨찾기 3색). **캔버스+PDF 툴바 양쪽** 올가미 버튼 바로 왼쪽 배치(양쪽 다 배치 실측). 클릭/탭=현재 펜에 즉시 적용, **우클릭(마우스)+길게 누르기 600ms(태블릿/터치)**=그 칸을 현재 펜 색으로 저장(+에메랄드 링 피드백, 길게 누른 뒤 이어지는 click은 억제). `preferences.favoriteColors`로 영속·양쪽 공유. 실브라우저 검증: 우클릭 저장(주황→#334155+localStorage), 터치 길게 누르기 저장(파랑), **짧은 탭은 저장 미발동+적용만**(획 51,65,85), PDF에서 클릭→파랑 획 (59,130,246).
