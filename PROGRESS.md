...
- **③ 페이지 관리(여러 장) — 완료·검증**: 노트 안 여러 페이지(삼성노트 #13). **DB 마이그레이션 0**으로 구현 — `InkStroke`/`InkSegment`에 `page?: number`(없으면 0=구노트 완전 호환, strokes jsonb에 그대로 실림). InkCanvas가 전체 획을 모델에 들고 **렌더·히트테스트(지우개/올가미/획탭)만 현재 페이지 필터** → CRDT erase append-only·undo/redo·유실0 구조 무변경. 델타에 page 포함(원격은 `ensurePageCount`로 페이지 자동 확장, `isInkDelta`는 구조상 통과 — syncEngine 무변경). 좌하단 `◀ n/N ▶ +` 네비(줌은 우하단). `highlightByTime`은 전 페이지 검색+매칭 페이지로 자동 점프.
  ...
  - **③-후속 예정**: **연속 스크롤**(페이지 스택처럼 아래로 조금 스크롤하면 다음 페이지 윗부분이 보이는 방식) — 요청됨. 현재 단일 캔버스/페이지 렌더라 **PDF뷰어처럼 페이지별 캔버스 스택으로 바꾸는 큰 엔진 변경** 필요(드로잉·레이어·올가미·줌 회귀 주의). 갤러리/플립과 별개 작업으로 신중히.
