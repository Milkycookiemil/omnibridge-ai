...
### 정직한 현재 상태 (실제 vs 시뮬) — 제품화의 출발점
- ❌ **노트가 실제로 영속 저장되지 않음** — 대시보드 "최근 노트"는 더미(`data.ts`). Drive는 업로드만 있고 읽기(list/download) 없음.
- ❌ AI 요약 카드 = 더미 (실 LLM 아님).
- 🟡 기기간 동기화 = Supabase Realtime 브로드캐스트(획 전송)만. DB 영속 없음, `persistSession:false`.
- ✅ 인증 · 필기 엔진(펜 5종/레이어/획·영역 지우개) · 온디바이스 Whisper 전사는 실제 동작.

### 제품화 로드맵
| 단계 | 내용 | 외부 준비물 |
|---|---|---|
| **1a** | 로컬 우선 실제 노트 CRUD (IndexedDB), 더미 제거 | 없음 |
| **1b** | Supabase 저장 + 계정별 격리(RLS), Firebase Third-Party Auth 연동 | Supabase 프로젝트 |
